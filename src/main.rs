    ...
    fn add_field(&mut self, field: Field<'a>) {
        match field.field_num {
            ...
            3 => {
                ...
                let mut data = field.value.as_bytes();
                while !data.is_empty() {
                    let parsed = parse_field(data);
                    match parsed.0.field_num {
                        1 => phone.number = parsed.0.value.as_str(),
                        2 => phone.type_ = parsed.0.value.as_str(),
                        _ => panic!("Unexpected field number: {}", parsed.0.field_num),
                    }
                    data = parsed.1;
                }
                self.phone.push(phone);
            }
            _ => panic!("Unexpected field number: {}", field.field_num),
        }
    }
}
